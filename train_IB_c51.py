...
# Paths
FAST_BUFFER_PATH  = Path(os.environ.get("SLURM_TMPDIR", "/tmp")) / "c51_expert_buffer"
PERM_BUFFER_PATH  = Path("buffers/c51_expert_buffer")
...
COLLECT_EPISODES      = 25    # episodes per data-collection phase per block
OPT_STEPS_PER_BURST   = 100      # optimiser steps per burst block gradually increase to 200
...
LOG_DIR, CKPT_DIR = Path("logs"), Path("checkpoints/full_expert_c51")
LOG_DIR.mkdir(parents=True,exist_ok=True)
CKPT_DIR.mkdir(parents=True,exist_ok=True)
ckpt_path = CKPT_DIR / "expert_latest.pt"
bkp_path  = CKPT_DIR / "expert_prev_latest.pt"
...
log_path = LOG_DIR / "expert_train_log.csv"
...
with log_path.open("a", newline="") as flog:
    ...
    for block in range(start_block, 1_000_000):
        ...
        for _ in tqdm(range(COLLECT_EPISODES), desc=f"Expert Collecting block {block}"):
            ...
            validation_obs, _ = validation_env.reset(options={"day": env.day, "user": env.profile})# reset validation env with same day and user profile
            ...
            use_optimal = True
            validation_use_optimal = True
            while not (done and  hybrid_done) and global_step < TOTAL_STEPS:

                if not done:
                    act = agent.act(state)
                    
                    if use_optimal:
                        _, optimal_actions, _ = env.helper_replay_benchmark(graph = env.graph)
                    ...
                    feasibility_penalty = data['feasibility_penalty']
                    has_arrived = data['reached_goal']
                    rlif_r = (1 + feasibility_penalty + has_arrived * 10) if act == expert_action else (-1 + feasibility_penalty + has_arrived * 10)
                    #agent.mem.push(obs, act, rlif_r, nxt, done)
                    episode_buffer.append(Transition(obs, act, rlif_r, nxt, done))
...
#---------------------------------------------------------
                ...
                if validation_use_optimal:
                    _, optimal_actions, _ = validation_env.helper_replay_benchmark(graph = validation_env.graph)
                    #print("Optimal actions:", optimal_actions)
                    if optimal_actions:
                        expert_action = optimal_actions[0]
                    else:
                        validation_use_optimal = False
                        continue  # recompute using fallback next loop
                if not validation_use_optimal:
                    if validation_env.current_soc < validation_env.goal_soc:
                        expert_action = 0
                    elif validation_env.current_soc > validation_env.goal_soc:
                        expert_action = 1
                    else:
                        expert_action = 2
                validation_nxt, validation_r, validation_done, _, validation_data = validation_agent.env.step(validation_act)
                validation_feasibility_penalty = validation_data['feasibility_penalty']
                validation_has_arrived = validation_data['reached_goal']
                validation_rlif_r = (1 + validation_feasibility_penalty + validation_has_arrived * 10) if validation_act == expert_action else (-1 + validation_feasibility_penalty+ validation_has_arrived * 10)
                ...
                validation_ep_reward += validation_rlif_r
                ...
                if not check:
                    ...
                    if hybrid_action_idx < len(hybrid_actions):
                        ...
                        if hybrid_env.current_soc < hybrid_env.goal_soc:
                            ...
                            hybrid_act = 2
                _, optimal_actions, _ = hybrid_env.helper_replay_benchmark(graph = hybrid_env.graph)
                expert_action = optimal_actions[0]
                # Step hybrid agent
                hybrid_nxt, hybrid_r, hybrid_done, _, hybrid_data = hybrid_env.step(hybrid_act)
                hybrid_has_arrived = hybrid_data['reached_goal']
                hybrid_rlif_r = (1 + hybrid_has_arrived*10) if hybrid_act == expert_action else (-1 + hybrid_has_arrived*10)
                hybrid_obs, hybrid_state = (hybrid_nxt, hybrid_agent.prep(hybrid_nxt)) if not hybrid_done else (None, None)
                hybrid_ep_reward += hybrid_rlif_r


                
            
            if not hybrid_has_arrived:
                print("⚠️ TRAIN Hybrid agent failed to reach goal:")
                print("  -H Day:", hybrid_env.day)
                print("  -H Profile:", hybrid_env.profile)
                print("  -H Final SoC:", hybrid_env.current_soc)
                print("  -H Goal SoC:", hybrid_env.goal_soc)
                print("  -H Time:", hybrid_env.current_time)

                
                print("  -V Day:", validation_env.day)
                print("  -V Profile:", validation_env.profile)
                print("  -V Final SoC:", validation_env.current_soc)
                print("  -V Goal SoC:", validation_env.goal_soc)
                print("  -V Time:", validation_env.current_time)

                print("  -E Day:", env.day)
                print("  -E Profile:", env.profile)
                print("  -E Final SoC:", env.current_soc)
                print("  -E Goal SoC:", env.goal_soc)
                print("  -E Time:", env.current_time)
        ...
        USER_PROFILES = env.users_pool
        #------------------test episode-----------------------------
        for _ in tqdm(range(TEST_COLLECT_EPISODES), desc = 'Expert Testing on Test DATA' ): #15 episodes per day since we have 15 users 
            user = USER_PROFILES[_ % len(USER_PROFILES)].copy()
            user[0], user[-1] = user[0] + random.choice([-0.25, 0.25]), user[-1] + random.choice([-1, 1])
            ...
            while not (test_done and hybrid_test_done):
                # -- Normal test agent --
                ...
                if test_use_optimal:
                    _, optimal_actions, _ = test_env.helper_replay_benchmark(graph = test_env.graph)
                    #print("Optimal actions:", optimal_actions)
                    if optimal_actions:
                        expert_action = optimal_actions[0]
                    else:
                ...
                test_feasibility_penalty = test_data['feasibility_penalty']

                test_rlif_r = (1 + test_feasibility_penalty + test_has_arrived*10) if test_act == expert_action else (-1 + test_feasibility_penalty + test_has_arrived*10)

                
                test_obs, test_state = (test_nxt, test_agent.prep(test_nxt)) if not test_done else (None, None)
                test_ep_reward += test_rlif_r
                ...
                if not check:
                    ...
                    if hybrid_action_idx < len(hybrid_test_actions):
                        ...
                        if hybrid_test_env.current_soc < hybrid_test_env.goal_soc:
                            ...
                            hybrid_test_act = 2
                _, optimal_actions, _ = hybrid_test_env.helper_replay_benchmark(graph = hybrid_test_env.graph)
                expert_action = optimal_actions[0]
                # Step hybrid agent
                ...
                hybrid_test_rlif_r = (1 + hybrid_test_has_arrived*10) if hybrid_test_act == expert_action else (-1 + hybrid_test_has_arrived*10)
                ...
                hybrid_test_ep_reward += hybrid_test_rlif_r
        ...
        if block % CHECK_BLOCK_INTERVAL == 0:
            ...
            dest_path = CKPT_DIR / f"expert_block_{block}_steps_{global_step}.pt"
