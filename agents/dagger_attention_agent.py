...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        # x: [B, N, C] where N = sequence length (flattened grid)
...
class DQNNet(nn.Module):
    def __init__(self, n_actions: int):
        ...
        # attention works on (B, N, C), here C=256 from last conv
        self.attn = SelfAttention(dim=256, heads=4)

        # project down before FC
    ...
    def forward(self, x):
        ...
        B, C, H, W = z.shape
        z = z.flatten(2).transpose(1, 2)       # [B, N, C], N=H*W
        z = self.attn(z)                       # [B, N, C]
        z = z.mean(dim=1)                      # pool over sequence
        return self.fc(z)                      # [B, A]
